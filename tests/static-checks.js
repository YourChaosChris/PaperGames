...
// Languages whose legal pages stay in English on purpose (the German and
// English versions of the Impressum and privacy policy are the binding
// ones): they may leave out the impressum_*/privacy_* texts, which fall
// back to English, but every other text must be there.
const LEGAL_ENGLISH_LANGS = ["ar"];
const LEGAL_KEY = /^(impressum|privacy)_/;
...
function checkI18nParity(STRINGS) {
  ...
  for (const lang of langs) {
    ...
    const legalEnglish = LEGAL_ENGLISH_LANGS.indexOf(lang) !== -1;
    const missing = [...enKeys].filter((k) => !keys.has(k) && !(legalEnglish && LEGAL_KEY.test(k)));
  ...
  if (bad.length) {
    ...
    ok(`i18n parity: ${langs.length} languages, ${enKeys.size} keys each` +
      (LEGAL_ENGLISH_LANGS.length ? ` (${LEGAL_ENGLISH_LANGS.join(", ")}: legal texts stay English)` : ""));
  }
}
