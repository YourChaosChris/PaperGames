...
const STRINGS = {
  en: {
    ...
    guide_web_body: "Open this same web address in your e-reader's browser and bookmark it. After the first visit, eInkChess caches itself for offline use automatically - close the WiFi and it keeps working. Only Chess's online Lichess mode needs an actual connection; Go is offline-only and unaffected.",
    ...
    guide_sideload_body: "Copy all the app's files onto the device over USB and open index.html directly from local storage (a file:// address). Offline play works exactly the same way. The one thing that doesn't work over file:// is Lichess login (OAuth requires a real http/https address) - local 2-player and vs-computer modes in both Chess and Go are unaffected.",
    guide_offline_title: "What works offline",
    guide_offline_body: "Everything except Chess's online Lichess games: local 2-player and the built-in computer opponent at every level, in both Chess and Go, run entirely on the device with no server involved - checkmate/stalemate/draw detection for Chess, and captures, scoring and komi for Go.",

    about_intro: "I searched for a simple chess game for my eReader — but all I found were people searching, not playing. So I built my own. That’s how eInkChess was born, and I’m happy to share it with everyone. It has since grown to include Go, with more E-Ink-friendly board games planned.",
