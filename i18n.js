...
const STRINGS = {
  en: {
    ...
    piece_queen: "Queen",
    piece_king: "King",
    term_castling: "Castling",
    term_en_passant: "En passant",
    term_promotion: "Promotion",
    term_checkmate: "Checkmate",
    term_stalemate: "Stalemate",
    term_threefold: "Threefold repetition",
    term_fifty_move: "Fifty-move rule",
    term_insufficient: "Insufficient material",

    rules_title: "Chess Rules",
    rules_intro: "A quick reference for how eInkChess plays the game - useful if you're rusty, or learning.",
    rules_movement_title: "How the pieces move",
    rules_pawn: "One square forward (two from its starting square), captures one square diagonally forward. Reaching the far side promotes it to any other piece (eInkChess always promotes to a queen).",
    rules_knight: "Moves in an L-shape (two squares one way, one square perpendicular) and is the only piece that can jump over others.",
    rules_bishop: "Any number of squares diagonally. Stays on one square color for the whole game.",
    rules_rook: "Any number of squares horizontally or vertically.",
    rules_queen: "Any number of squares in any direction - combines the rook and bishop.",
    rules_king: "One square in any direction. Can never move into check.",
    rules_special_title: "Special moves",
    rules_castling: "The king moves two squares toward a rook, and that rook jumps to the square beside the king. Only if neither piece has moved yet, the squares between them are empty, and the king isn't in, through, or ending in check.",
    rules_en_passant: "If an enemy pawn advances two squares and lands beside yours, your pawn may capture it as if it had only moved one square - but only immediately on the next move.",
    rules_promotion: "A pawn reaching the last rank becomes another piece, almost always a queen.",
    rules_end_title: "How a game ends",
    rules_checkmate: "The king is in check with no legal move to escape it. That side loses.",
    rules_stalemate: "The side to move has no legal move and isn't in check. The game is a draw.",
    rules_draws_title: "Draws by rule",
    rules_threefold: "The exact same position occurs three times.",
    rules_fifty_move: "Fifty moves pass (by both sides) with no pawn move and no capture.",
    rules_insufficient: "Neither side has enough pieces left to possibly checkmate (e.g. king vs. king, or king and bishop vs. king).",
  ...
  de: {
    ...
    home_play_button: "▶ Schach spielen",
    ...
    piece_queen: "Dame",
    piece_king: "König",
    term_castling: "Rochade",
    term_en_passant: "En passant",
    term_promotion: "Bauernumwandlung",
    term_checkmate: "Schachmatt",
    term_stalemate: "Patt",
    term_threefold: "Dreifache Stellungswiederholung",
    term_fifty_move: "50-Züge-Regel",
    term_insufficient: "Unzureichendes Material",

    rules_title: "Schachregeln",
    rules_intro: "Eine kurze Übersicht, wie eInkChess Schach spielt – nützlich zum Auffrischen oder Lernen.",
    rules_movement_title: "Wie die Figuren ziehen",
    rules_pawn: "Ein Feld vorwärts (vom Startfeld aus zwei), schlägt ein Feld diagonal vorwärts. Erreicht er die gegnüberliegende Grundreihe, wird er umgewandelt (eInkChess wandelt immer in eine Dame um).",
    rules_knight: "Zieht im L-Muster (zwei Felder in eine Richtung, ein Feld quer dazu) und ist die einzige Figur, die andere überspringen kann.",
    rules_bishop: "Beliebig viele Felder diagonal. Bleibt das ganze Spiel über auf einer Feldfarbe.",
    rules_rook: "Beliebig viele Felder waagerecht oder senkrecht.",
    rules_queen: "Beliebig viele Felder in jede Richtung – vereint Turm und Läufer.",
    rules_king: "Ein Feld in jede Richtung. Darf sich nie ins Schach ziehen.",
    rules_special_title: "Sonderzüge",
    rules_castling: "Der König zieht zwei Felder auf einen Turm zu, dieser Turm springt auf das Feld daneben. Nur möglich, wenn beide Figuren noch nicht gezogen haben, die Felder dazwischen frei sind und der König weder im Schach steht noch durch ein bedrohtes Feld zieht oder im Schach landet.",
    rules_en_passant: "Zieht ein gegnerischer Bauer zwei Felder vor und landet neben deinem, darfst du ihn so schlagen, als wäre er nur ein Feld gezogen – aber nur direkt im nächsten Zug.",
    rules_promotion: "Ein Bauer, der die letzte Reihe erreicht, wird zu einer anderen Figur, fast immer einer Dame.",
    rules_end_title: "Wie eine Partie endet",
    rules_checkmate: "Der König steht im Schach und es gibt keinen legalen Zug, das zu ändern. Diese Seite verliert.",
    rules_stalemate: "Die Seite am Zug hat keinen legalen Zug und steht nicht im Schach. Die Partie ist remis.",
    rules_draws_title: "Remis nach Regel",
    rules_threefold: "Die exakt gleiche Stellung tritt dreimal auf.",
    rules_fifty_move: "50 Züge (beider Seiten) vergehen ohne Bauernzug und ohne Schlagen.",
    rules_insufficient: "Keine Seite hat noch genug Material, um überhaupt matt setzen zu können (z. B. König gegen König, oder König und Läufer gegen König).",
