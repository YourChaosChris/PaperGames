// tablut-ai.js
// Offline opponent for Tablut, structurally identical to hnefatafl-ai.js
// (see that file's own header) but re-tuned for a 9x9 board, which has
// a noticeably larger branching factor than the 7x7 Brandub board (more
// squares, longer rook-style slides) - the wall-clock deadline below
// matters even more here for staying responsive on a slow e-reader:
...
const TablutAi = (function () {
  const INF = 1e9;
  const PIECE_VALUE = 100;
  const KING_VALUE = 100000;
  const CORNERS = TablutCore.CORNERS;
  const SIZE = TablutCore.SIZE;
  // The center (the king's own starting square) is the furthest any
  // square can be from its nearest corner on this board (distance 8),
  // so this constant is always >= any real cornerDistance() result -
  // it just needs to keep the escape-race score positive, the way
  // hnefatafl-ai.js's own "12" does for its smaller board.
  const MAX_CORNER_DIST = 2 * (SIZE - 1);
  ...
  function evaluateFor(side, state) {
    ...
    const opponent = TablutCore.otherPlayer(side);
    let score = 0;
    let kingPos = null;
    ...
    if (kingPos) {
      ...
      const escapeScore = (MAX_CORNER_DIST - dist) * 4;
      score += (side === "defender" ? 1 : -1) * escapeScore;
    }
    const myMoves = TablutCore.getLegalMoves(state, side).length;
    const oppMoves = TablutCore.getLegalMoves(state, opponent).length;
  ...
  function countEnemyNeighbors(board, [r, c], player) {
    const opponent = TablutCore.otherPlayer(player);
    let n = 0;
    [[-1, 0], [1, 0], [0, -1], [0, 1]].forEach(([dr, dc]) => {
      const nr = r + dr, nc = c + dc;
      if (TablutCore.inBounds(nr, nc)) {
  ...
  function minimax(state, playerToMove, depth, maxDepth, alpha, beta, perspective, searchState) {
    ...
    const moves = TablutCore.getLegalMoves(state, playerToMove);
    if (!moves.length) {
    ...
    for (const m of ordered) {
      const next = TablutCore.applyMove(state, playerToMove, m);
      const score = minimax(next, TablutCore.otherPlayer(playerToMove), depth + 1, maxDepth, alpha, beta, perspective, searchState);
  ...
  function searchBestMove(state, side, moves, maxDepth, nodeBudget, timeBudgetMs) {
    ...
    const rootScored = ordered.map((m) => ({
      move: m,
      score: evaluateFor(side, TablutCore.applyMove(state, side, m)) + (Math.random() - 0.5) * 2
    }));
    ...
    for (let depth = 1; depth <= maxDepth; depth++) {
      ...
      for (const m of ordered) {
        const next = TablutCore.applyMove(state, side, m);
        const score = minimax(next, TablutCore.otherPlayer(side), 1, depth, alpha, INF, side, searchState);
  ...
  const LEVEL_CONFIG = {
    ...
    3: { style: "search", maxDepth: 3, nodeBudget: 45000, timeBudgetMs: 2500 }
  };

  function chooseMove(state, side, level) {
    const moves = TablutCore.getLegalMoves(state, side);
    ...
    if (config.style === "greedy") {
      const scored = moves.map((m) => ({
        move: m,
        score: evaluateFor(side, TablutCore.applyMove(state, side, m))
      }));
...
if (typeof module !== "undefined" && module.exports) {
  module.exports = TablutAi;
}
if (typeof window !== "undefined") {
  window.TablutAi = TablutAi;
}
